...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="gusehdqkd/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/bangddong/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/bangddong/guestbook-config.git'
                }
            }
        }
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_gusehdqkd') {
                        oDockImage.push()
                    }
                }
            }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://bangddong:${GITHUB_ACCESS_TOKEN}@github.com/bangddong/guestbook-config.git"
                    '''
                }
            }
        }
